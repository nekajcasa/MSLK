...
class Meritev:
    ...
    def naredi_meritev(self):
        ...
        #laser
        self.task.ai_channels.add_ai_voltage_chan(self.ch_laser,min_val = self.U_min, max_val=self.U_max)
        #sila če je f2=0 pomeni da ni nabojnega ojačevalnka-> merimo napetost
        if self.f2<=0:
            #potrebno je se dodati!
            pass
        else:
            self.task.ai_channels.add_ai_force_iepe_chan(self.ch_silomer,sensitivity=self.f1*self.f2,min_val=self.min_val_silomer,max_val=self.max_val_silomer)
        self.task.timing.cfg_samp_clk_timing(self.frekvenca, sample_mode=nidaqmx.constants.AcquisitionType.FINITE, samps_per_chan=self.st_vzorcev)
...
class Scanner:
    ...
    def narisi_tarce(self,img,tarče):
        ...
        for i in tarče:
            ...
            cv2.rectangle(img,start_point,end_point,(140, 225, 70),thickness = 5)
    ...
    def dolocanje_tarc(self):
        ...
        while run==1:
            ...
            if move_str=='exit':
                ...
                self.k = self.laser.kalibracija_basic(1,1,položaj_zrcal[0],položaj_zrcal[1])
            elif move_str=="b":
                if len(tarče) != 0:
                    tarče = tarče[:-1]
                    st_tarče -= 1
        ...
        if self.tarče!="exit":
            ...
            while i<len_c:
                if np.any(self.tarče[i]<0) or self.tarče[i,0]>=img0.shape[1] or self.tarče[i,0]>=img0.shape[0]:
                    ...
                    time.sleep(1)
                    img0 = img1
                    self.plotimg(self.narisi_tarce(self.kamera.req("img"), self.tarče))
                    i +=1
                    vzorec += 1
    ...
    def img_translation(self,im1, im2):
       ...
       cor = scipy.signal.fftconvolve(im1_gray, im2_gray[::-1,::-1], mode='same')
       brightest=np.unravel_index(np.argmax(cor), cor.shape)
       
       sz = im1.shape
       translation=np.array([sz[1]/2-float(brightest[1]),sz[0]/2-float(brightest[0])])
...
if __name__=='__main__':
    ...
    pi_kamera.connect()
    #kalibracija laserja
    scanner.k = laser.kalibracija_basic(2, 2, položaj_zrcal[0], položaj_zrcal[1])
    #prikaz prve slike
